#!/usr/bin/env python3
"""
Automated Static Malware Analysis with Cloud Integration(AWS)
"""
...
class AWSMalwareAnalyzer:
    """Main class for automated malware analysis on AWS"""
    
    def __init__(self, config_file: str = 'config.json'):
        """Initialize AWS connections and configuration"""
...
class StaticAnalyzer:
    """Performs comprehensive static analysis of malware samples"""
    
    # Known benign installer URLs (should not be flagged as C2)
    ...
    # Standard PE section names (non-suspicious)
    STANDARD_SECTIONS = ['.text', '.data', '.rdata', '.bss', '.idata', '.edata', 
                         '.rsrc', '.reloc', '.pdata', '.tls', '.CRT', 'CODE', 'DATA']
    
    # BEHAVIOR EXPECTATION MATRIX: What's normal for each file role
    # This is the core of semantic analysis - behaviors must match role
    BEHAVIOR_EXPECTATIONS = {
        "installer": {
            # These behaviors are EXPECTED and should NOT add to risk score
            "expected": [
            ...
            # These behaviors are SUSPICIOUS even for installers
            "suspicious": [
                "process_injection", "credential_theft", "keylogging",
                "browser_hooking", "crypto_mining"
            ],
            # These would be CONTRADICTIONS (installer shouldn't do these)
            "contradictions": [
        ...
        "loader_dropper": {
            # Everything is suspicious for loaders
            "expected": [],
            "suspicious": [
        ...
        "unknown": {
            # Conservative scoring when role is unknown
    ...
    def __init__(self, malware_path: str, config: Dict = None):
        """Initialize static analyzer with malware sample path and optional config"""
        ...
        self.results = {
            ...
            "anti_analysis": {},
            "language_detection": {},
            # Industry-grade analysis additions
            "rich_header": {},
            "pdb_info": {},
            "tls_callbacks": {},
            "overlay": {},
    ...
    def analyze_all(self) -> Dict:
        ...
        # Industry-grade analysis (works on any file type)
        self.scan_yara_rules()
        self.detect_config_blobs()
        self.detect_instruction_anti_debug()
        self.correlate_threat_intel()
        
        # Context-aware analysis (NEW) - must run before risk scoring
        self.detect_installer_framework()
        self.classify_file_role()  
        self.analyze_api_clusters()       # NEW: Intent-based API analysis
        self.calculate_benign_indicators()
        self.detect_malware_indicators()
        self.detect_contradictions()      # NEW: Contradiction detection
        
        # Final risk scoring (now justification-based, contradiction-aware)
        self.calculate_risk_score()
        
        # Generate "Why it is suspicious" explanations
        self.generate_suspicion_report()
        
        return self.results
    
    def get_file_metadata(self):
        """Extract file metadata and identification"""
        try:
    ...
    def calculate_hashes(self):
        """Calculate multiple cryptographic hashes and fuzzy hash"""
        try:
    ...
    def extract_strings(self):
        ...
        try:
            ...
            self.results["strings"] = {
                ...
                # SPLIT: Payment indicators affect scoring, primitives are informational
                ...
                # Context-aware keywords for role-based interpretation
    ...
    def extract_unicode_strings(self, min_length: int = 4) -> List[str]:
        ...
        try:
            ...
            # Look for UTF-16 LE strings (common in Windows)
        ...
        return strings[:500]  # Limit to first 500 strings
    
    def categorize_strings(self, strings: List[str]) -> Dict[str, List[str]]:
        """Categorize strings by type (with false positive filtering and semantic awareness)"""
        categories = {
            "interesting": [],
            "urls": [],
            "ips": [],
            ...
            "paths": [],
            "emails": [],
            # SPLIT: crypto_payment (malicious) vs crypto_primitives (informational only)
            "crypto_payment": [],      
            "crypto_primitives": [],   
            "api_calls": [],
            "dlls": [],
            "whitelisted_urls": [],
            # NEW: Context-aware keyword tracking (for contradiction detection, NOT scoring)
            "context_keywords": []     
        }
        ...
        # Registry pattern - only flag specific persistence paths, NOT generic HKLM/HKCU
        ...
        # Version pattern (to filter false positives like "1.0.0.0" in XML)
        xml_context_patterns = ['version=', 'encoding=', '<?xml', 'xmlns', 'assembly']
        
        # Shell extension patterns - these are NOT crypto, NOT suspicious
        shell_extension_patterns = [
            'ShellEx', 'PropertySheetHandlers', 'ContextMenuHandlers', 
            'IconHandler', 'CopyHookHandlers', 'DragDropHandlers'
        ]
        
        # These are tracked for contradiction detection only
        context_aware_keywords = {
            # keyword: {role: interpretation}
        ...
        # Keywords that ARE suspicious regardless of context (but still don't score directly)
        suspicious_keywords = [
            'ransom', 'exploit', 'backdoor', 'keylog', 'steal', 
            'inject', 'hook', 'c2', 'beacon', 'shellcode'
        ]
        
        # Common Windows API calls (informational)
        api_keywords = [
        ...
        # CRYPTO SPLIT:
        # Payment indicators = ransomware/crypto-theft (affects scoring)
        ...
        # Primitives = hash algorithms, crypto APIs (informational only, NO scoring)
        ...
        for string in strings:
            ...
            # Skip shell extension paths entirely - they are NOT suspicious
            if any(sep in string for sep in shell_extension_patterns):
                continue
            
            # URLs - check for whitelisted domains
            ...
            # IP addresses - filter out version numbers and XML context
            ...
            # Registry keys (persistence only)
            ...
            # Suspicious keywords - track in interesting but DO NOT score directly
        ...
        for key in categories:
            if key != "context_keywords":  # context_keywords is a list of dicts
                categories[key] = list(set(categories[key]))[:50]
            else:
                # Dedupe context_keywords by string
                seen = set()
                unique = []
    ...
    def analyze_pe_file(self):
        """Analyze PE file structure (installer-aware)"""
        try:
            ...
            # Check for common PE anomalies (installer-aware)
            anomalies = []
            is_installer = self.results.get("installer_info", {}).get("is_installer", False)
            
            # Check for suspicious section names
            # NOTE: .rsrc is NOT suspicious - it's a standard Windows resource section
            # Many installers legitimately store compressed payloads in .rsrc
    ...
    def is_section_suspicious(self, section) -> bool:
        """Check if a PE section is suspicious"""
    ...
    def analyze_resources(self):
        ...
        try:
            ...
            self.results["resources"] = {
                ...
                "resources_list": resources[:20]  # Limit to first 20 resources
            }
    ...
    def detect_packers(self):
        ...
        try:
            # Use YARA rules for packer detection
            packer_rules = """
            rule UPX_Packer {
                strings:
                    $upx1 = "UPX!"
            ...
            # Check PE entropy for packing indicators
            if self.is_pe_file():
                try:
                    pe = pefile.PE(str(self.malware_path))
                    
                    # Check for high entropy in entry point section
                    for section in pe.sections:
                    ...
                    # Check for few imports (common in packed files)
                    if hasattr(pe, 'DIRECTORY_ENTRY_IMPORT'):
    ...
    def get_hex_dump(self, num_bytes: int = 256):
        ...
        try:
            ...
            # Extract first 10 hex digits
            first_10_hex = ''.join(f'{b:02x}' for b in data[:5])  # 5 bytes = 10 hex digits
    ...
    def calculate_entropy(self):
        ...
        try:
            ...
            # Calculate entropy for different parts of the file
    ...
    def classify_entropy(self, entropy: float) -> str:
        """Classify entropy level (context-aware language)"""
        # NOTE: High entropy indicates randomness/compression, NOT necessarily encryption
        # Installers and containers (MSI, CAB) naturally have high entropy due to compression
        is_installer = self.results.get("installer_info", {}).get("is_installer", False)
    ...
    def detect_anti_analysis(self):
        """Detect anti-analysis and anti-VM techniques (context-aware)"""
        try:
            anti_techniques = []
            is_container = not self.is_pe_file()
            ...
            # Anti-debugging API strings (only meaningful in PE files)
            if not is_container:
                anti_debug_strings = [
                    b'IsDebuggerPresent',
            ...
            # Anti-VM strings - in containers, these are often just product names
            # Only flag in PE files where they indicate VM detection code
            if not is_container:
                anti_vm_strings = [
                    b'VMware',
                    b'VirtualBox',
            ...
            # Anti-sandbox strings - check for both container and PE
            anti_sandbox = [
                b'SbieDll.dll',
            ...
            # Process and service detection (only meaningful in PE context)
            if not is_container:
                process_detection = [
                    b'wireshark',
                    b'fiddler',
            ...
            # Time-based checks (only PE - these are API names)
            if not is_container:
                timing_checks = [
                    b'GetTickCount',
    ...
    def detect_programming_language(self):
        """Detect programming language and compiler (PE FILES ONLY)"""
        try:
            # Language detection only makes sense for PE executables
            # Container files (MSI/CAB/ZIP) contain embedded binaries compiled with various tools
    ...
    # ============================================================================
    # INDUSTRY-GRADE ANALYSIS METHODS
    # ============================================================================
    
    def analyze_rich_header(self):
        """Parse and analyze PE Rich Header for attribution"""
        try:
            pe = pefile.PE(str(self.malware_path))
            ...
            if hasattr(pe, 'RICH_HEADER') and pe.RICH_HEADER:
                ...
                # Calculate Rich Header hash (XOR key is part of the header)
                rich_data = pe.get_data(0x80, 0x80)  # Typical Rich Header location
                if rich_data:
                ...
                if hasattr(pe.RICH_HEADER, 'values'):
                    for entry in pe.RICH_HEADER.values:
                        # Rich header format: (tool_id << 16) | build_version
    ...
    def _identify_rich_tool(self, tool_id: int, build_ver: int) -> str:
        """Identify compiler/tool from Rich Header IDs"""
        tool_map = {
            0: "Unknown",
            1: "Import Table",
            2: "Linker",
            ...
            if hasattr(pe, 'DIRECTORY_ENTRY_DEBUG'):
                for debug_entry in pe.DIRECTORY_ENTRY_DEBUG:
                    if debug_entry.struct.Type == 2:  # IMAGE_DEBUG_TYPE_CODEVIEW
                        ...
                        # Check for RSDS signature (CodeView PDB 7.0)
                        if debug_data[:4] == b'RSDS':
                            ...
                            # Extract potential developer info
                        ...
                        # Check for NB10 signature (CodeView PDB 2.0)
    ...
    def enumerate_tls_callbacks(self):
        """Enumerate TLS (Thread Local Storage) callbacks"""
        try:
            pe = pefile.PE(str(self.malware_path))
            ...
            if hasattr(pe, 'DIRECTORY_ENTRY_TLS'):
                ...
                # Only flag pre_entry_execution if we actually found callbacks
                if len(tls_info["callbacks"]) > 0:
                    ...
                    # Has TLS directory but no callbacks - common in Delphi/certain compilers
    ...
    def analyze_overlay(self):
        """Analyze overlay (appended data after PE sections)"""
        try:
            pe = pefile.PE(str(self.malware_path))
            ...
            # Calculate overlay offset (end of last section)
            ...
            if overlay_offset and overlay_offset < file_size:
                ...
                if overlay_size > 0:
                    ...
                    with open(self.malware_path, 'rb') as f:
                        ...
                        overlay_data = f.read(min(overlay_size, 1024 * 1024))  # Read up to 1MB
                    ...
                    # Flag as suspicious ONLY if high entropy AND NOT an installer payload
    ...
    def analyze_version_info(self):
        """Analyze RT_VERSION resource for file information (context-aware)"""
        try:
            pe = pefile.PE(str(self.malware_path))
            ...
            version_info = {
                ...
                "mismatch_type": None,  # "benign_rename" or "suspicious_masquerade"
                "all_fields": {}
            }
            ...
            # Check for filename mismatch - distinguish benign from malicious
            if version_info["original_filename"]:
                ...
                if original_name and original_name != actual_name:
                    ...
                    # Determine if this is benign rename or suspicious masquerade
                    # Benign: Same product, different wrapper name (BraveUpdateSetup.exe → BraveBrowserSetup.exe)
                    # Suspicious: Different product entirely (calc.exe pretending to be svchost.exe)
                    
                    # Extract base names without extensions
                    actual_base = actual_name.rsplit('.', 1)[0].lower()
                    original_base = original_name.rsplit('.', 1)[0].lower()
                    
                    # Check for shared root (e.g., "brave" in both names)
                    shared_words = set(actual_base.split()) & set(original_base.split())
                    product_name = (version_info.get("product_name") or "").lower()
                    
                    # Benign indicators:
                    # 1. Names share common words
                    # 2. Product name appears in actual filename
                    # 3. Names have similar structure (setup, installer, update)
    ...
    def map_capabilities(self):
        """Map API imports to high-level behaviors (CAPA-style) with confidence scoring"""
        try:
            pe = pefile.PE(str(self.malware_path))
            
            # Define behavior categories with REQUIRED APIs (must have multiple for high confidence)
            # Format: { behavior: { "required_count": N, "apis": [...], "corroborating": [...] } }
            behavior_definitions = {
                "keylogging": {
                    "required_count": 2,  # Need at least 2 keylogging-specific APIs
                    "primary_apis": ["GetAsyncKeyState", "GetKeyState", "GetKeyboardState",
                                    "SetWindowsHookExA", "SetWindowsHookExW", "GetRawInputData",
                                    "RegisterRawInputDevices"],
                    "corroborating": ["WriteFile", "send", "InternetOpen"]  # Evidence of exfil
                },
                "screen_capture": {
                    "required_count": 3,  # Need 3+ APIs for confidence (BitBlt+GetDC alone is normal GUI)
                    ...
                    "corroborating": ["WriteFile", "CreateFile", "InternetOpen", "send",
                                     "GdipSaveImageToFile", "SaveDC"]  # Evidence of capture+save
                },
                "clipboard_access": {
                    "required_count": 2,
                ...
                "process_injection": {
                    "required_count": 2,  # At least 2 of: VirtualAllocEx, WriteProcessMemory, CreateRemoteThread
                    ...
                    "corroborating": ["OpenProcess"]  # Must also have process handle
                },
                # REMOVED code_injection - VirtualAlloc/VirtualProtect alone is NOT injection
                # These are normal memory management APIs used by all runtime libraries
                # NOTE: Registry writes alone are NOT persistence
                # Persistence requires writing to AUTORUN locations
                "persistence": {
                    "required_count": 1,
                    # Only flag SERVICE creation as persistence - this is always intentional autorun
                    "primary_apis": ["CreateServiceA", "CreateServiceW"],
                    "corroborating": []
                },
                # New category for installer registry activity (not malicious)
                "registry_activity": {
                    "required_count": 1,
                ...
                "privilege_escalation": {
                    "required_count": 2,  # Need token manipulation + privilege lookup
                ...
                "credential_theft": {
                    "required_count": 1,  # Any of these is highly suspicious
                    "primary_apis": ["CredEnumerate", "CredRead", "LsaRetrievePrivateData",
                                    "CryptUnprotectData", "SamConnect", "SamQueryInformationUser"],
                    "corroborating": []
                },
                # Anti-debug: Common in Chromium/debug builds - low severity for installers
                "anti_debug": {
                    "required_count": 2,  # Need multiple to be concerning
                ...
                "timing_evasion": {
                    "required_count": 2,  # Need multiple timing APIs to be suspicious
                ...
                "dll_loading": {
                    "required_count": 2,  # LoadLibrary + GetProcAddress is common pattern
                ...
                "ransomware_behavior": {
                    "required_count": 2,  # Need crypto + file operations
                ...
                # NEW: File enumeration (worm/ransomware indicator)
                "file_enumeration": {
                    "required_count": 2,
                ...
                # NEW: File destruction/modification
                "file_destruction": {
                    "required_count": 1,
                ...
                # NEW: Process termination (used to kill security software)
                "process_termination": {
                    "required_count": 1,
            ...
            if hasattr(pe, 'DIRECTORY_ENTRY_IMPORT'):
                ...
                # Check each behavior category with confidence scoring
                for behavior, definition in behavior_definitions.items():
                    ...
                    if len(primary_matched) >= definition["required_count"]:
                        ...
                        # Special case: GUI apps commonly use BitBlt/GetDC
                ...
                # Generate risk indicators (only for high confidence behaviors)
                ...
                for behavior in capabilities["detected_behaviors"]:
                    if behavior in high_risk_behaviors:
                        ...
                        if conf in ["high", "medium"]:
                            ...
                            # Even low confidence ransomware is worth noting
                            capabilities["risk_indicators"].append(
    ...
    def scan_yara_rules(self, rules_path: str = None):
        """Scan file with YARA rules for malware family identification"""
        try:
            yara_results = {
                "matches": [],
                "malware_families": [],
            ...
            for source, rules in compiled_rules:
                try:
                    ...
                    for match in matches:
                        ...
                        # Try to extract malware family from rule name or meta
    ...
    def analyze_authenticode(self):
        """Analyze Authenticode digital signature"""
        try:
            pe = pefile.PE(str(self.malware_path))
            ...
            # Check for security directory (contains signature)
            ...
            if security_dir.VirtualAddress != 0 and security_dir.Size != 0:
                ...
                # Try to extract signer info using signify or openssl if available
                try:
                    ...
                    # osslsigncode not installed, try alternative
                    try:
                        result = subprocess.run(
    ...
    def categorize_imports_by_behavior(self):
        """Categorize imports by behavior type"""
        try:
            pe = pefile.PE(str(self.malware_path))
    ...
    def analyze_entry_point(self):
        """Analyze entry point for suspicious patterns"""
        try:
            pe = pefile.PE(str(self.malware_path))
            ...
            # Get first bytes at entry point
            try:
    ...
    def detect_config_blobs(self):
        """Detect potential encoded data (context-aware for containers)"""
        try:
            with open(self.malware_path, 'rb') as f:
                data = f.read()
            ...
            # Add context note for containers
            ...
            # Look for Base64 encoded blobs (only for PE files - too many FPs in containers)
            if not is_container:
                b64_pattern = re.compile(b'[A-Za-z0-9+/]{40,}={0,2}')
                b64_matches = b64_pattern.findall(data)
                for match in b64_matches[:5]:  # Limit to 5
                    try:
                        import base64
            ...
            # Find high-entropy regions (skip for containers - all regions are high entropy)
            if not is_container:
                chunk_size = 256
    ...
    def detect_instruction_anti_debug(self):
        """Detect instruction-level anti-debug techniques (PE FILES ONLY)"""
        try:
            # CRITICAL: Instruction analysis only makes sense for PE executables
            # Running this on MSI/CAB/ZIP/OLE containers produces hallucinated results
            # because random compressed bytes match x86 opcode patterns
            ...
            # Filter out likely padding
            anti_debug["int3_count"] = min(int3_count, 100)  # Cap display
            if int3_count > 10 and int3_count < 1000:  # Suspicious range
            ...
            # IsDebuggerPresent call pattern (indirect)
    ...
    def correlate_threat_intel(self):
        """Correlate with threat intelligence - includes VirusTotal API lookup"""
        try:
            ...
            # Check for known malicious patterns in strings
            ioc_patterns = {
    ...
    def detect_installer_framework(self):
        """Detect if file is a known installer framework (NSIS, MSI, InnoSetup, etc.)"""
        try:
            installer_info = {
                "is_installer": False,
                "framework": None,
            ...
            with open(self.malware_path, 'rb') as f:
                file_data = f.read()
                file_start = file_data[:4096]  # First 4KB for magic checks
            ...
            # === NSIS Detection ===
            nsis_indicators = 0
            # NSIS magic in overlay (0xEFBEADDE)
            ...
            # === MSI Detection ===
            if not installer_info["is_installer"]:
                # OLE compound file with MSI signature
            ...
            # === 7-Zip SFX Detection (CRITICAL - Firefox uses this) ===
            if not installer_info["is_installer"]:
                sfx7z_indicators = 0
                # 7z magic bytes: 7z¼¯' (hex: 37 7A BC AF 27 1C)
                # Can appear in overlay or file body
                ...
                # 7-Zip has distinctive error messages
                ...
                # Large overlay is essential for SFX
            ...
            # === UPX + Large Overlay = SFX Archive (Firefox, etc.) ===
            if not installer_info["is_installer"]:
                ...
                # UPX sections indicate UPX packing
                ...
                # UPX + large overlay = almost always SFX installer
            ...
            # === RAR SFX Detection ===
            if not installer_info["is_installer"]:
                # RAR magic: Rar!\x1a\x07\x00 (v4) or Rar!\x1a\x07\x01\x00 (v5)
            ...
            # === Generic Installer Heuristics ===
            if not installer_info["is_installer"]:
                generic_indicators = 0
                
                # MSCF (Microsoft CAB) overlay is a strong installer indicator
                ...
                # Version info suggests installer
                ...
                # Large overlay is common in installers
                if overlay.get("has_overlay") and overlay.get("overlay_ratio", 0) > 30:
                    generic_indicators += 1
                ...
                if generic_indicators >= 2:
                    ...
                    installer_info["confidence"] = min(0.9, generic_indicators / 4)  # Higher confidence with MSCF
    ...
    def classify_file_role(self):
        """Classify file into semantic role BEFORE scoring"""
        try:
            role_info = {
                "role": "unknown",
                "confidence": "low",
            ...
            # === Priority 1: Installer Detection ===
            ...
            if pe_header:
                ...
                try:
                    ...
                    if chars_int & 0x2000:  # IMAGE_FILE_DLL
            ...
            # === Priority 3: SFX/Installer Pattern (Packer + Large Overlay) ===
            ...
            # === Priority 4: Loader/Dropper Detection ===
            ...
            # Small executable + network + file writes = potential dropper
            ...
            # === Priority 5: Standard Application ===
            else:
                version_info = self.results.get("version_info", {})
    ...
    def is_windows_system_tool(self) -> bool:
        """
        Detect known Windows system tools that legitimately use sensitive APIs.
        These are core OS utilities that should not be flagged for their expected behavior.
        """
        WINDOWS_SYSTEM_TOOLS = {
            # Network utilities (legitimate network API usage)
            ...
            # System management (legitimate process/registry/file APIs)
            ...
            # Windows Media (legitimate COM APIs)
            'wmplayer.exe', 'wmpnscfg.exe', 'wmpnetwk.exe', 'wmpshare.exe', 'wmpconfig.exe',
            # System components with known FPs
        ...
        # Check for Windows system path in version info
        ...
        # Microsoft signed = system tool trust
    ...
    def is_known_admin_tool(self) -> bool:
        """
        Detect signed admin/debugger tools that legitimately access sensitive APIs.
        These tools (Sysinternals, debuggers, AV) should NOT be flagged for:
        - Credential APIs (they display security context)
        - Process APIs (they inspect processes)
        - Debug APIs (they are debuggers)
        """
        signals = []
        
        # Check for Sysinternals signature patterns
        ...
        # Check version info for known tool vendors
        version_info = self.results.get("version_info", {})
        ...
        # Check for heavy GUI imports (admin tools typically have rich UI)
        ...
        # Decision: Need 2+ signals to be considered admin tool
        is_admin_tool = len(signals) >= 2
        
        # Store result for use in other functions
    ...
    def analyze_api_clusters(self):
        """
        Analyze API combinations to determine INTENT, not just capability.
        This replaces individual API flagging with cluster-based analysis.
        """
        try:
            # Define API clusters that indicate specific INTENT
            CLUSTERS = {
                "injection": {
                ...
                "debugger": {
                    ...
                    "negative": {"WriteProcessMemory", "CreateRemoteThread"},  # Debuggers read, don't inject
                ...
                "admin_tool_cred_display": {
                    # Admin tools access credentials for DISPLAY, not theft
                    "required": {"CredEnumerate", "CredRead"},
                    "context": "admin_tool",  # Only applies if is_known_admin_tool() is True
                    "intent": "Credential enumeration for display (admin tool behavior)",
                    "weight": 0  # Benign in admin tool context
                }
            }
            ...
            for cluster_name, cluster_def in CLUSTERS.items():
                ...
                if len(required_matches) >= len(required) * 0.8:  
                    # Check negative conditions (disqualifying APIs)
                    if negative and (negative & all_apis):
                        continue  # Skip if has disqualifying APIs
    ...
    def calculate_benign_indicators(self):
        """Calculate evidence that suggests file is NOT malware"""
        try:
            benign_info = {
                "benign_score": 0,
                "evidence": [],
            ...
            # Check if persistence APIs are used WITH actual targets
    ...
    def detect_malware_indicators(self):
        """Detect HARD malware indicators that should always contribute to risk score"""
        try:
            indicators = {
                "hard_indicators": [],
            ...
            # Get actual string lists, not dictionary representation
            ...
            crypto_strings = strings_data.get("crypto_payment", [])  # Only payment indicators affect scoring
            api_calls = strings_data.get("api_calls", [])
            registry_keys = strings_data.get("registry_keys", [])
            
            # Combine all strings for searching (crypto_primitives is EXCLUDED - informational only)
            all_string_content = " ".join(interesting_strings + all_urls + crypto_strings)
            
            # === 1. INTERESTING STRINGS ALREADY DETECTED ===
            # These were already filtered for suspicious keywords in categorize_strings
            ...
            # === 2. SUSPICIOUS URLs (non-whitelisted) ===
            if all_urls:
            ...
            # Unsigned + No version info = suspicious (unless installer)
            if not is_installer:
            ...
            # High entropy without being an installer = suspicious
            ...
            # Fake version info (claims to be Microsoft but unsigned)
            company = str(version_info.get("company_name", "")).lower()
            ...
            # === 6. INSTRUCTION-LEVEL ANTI-DEBUG ===
            ...
            high_risk_caps = {
                ...
                "file_enumeration": 10  # Only suspicious in combination
            }
            
            for behavior, points in high_risk_caps.items():
                if behavior in detected_behaviors:
                    # file_enumeration only counts if combined with crypto or destruction
            ...
            # === 8. TLS CALLBACKS (pre-entry execution) ===
            ...
            # === 10. SUSPICIOUS IMPORTS COMBO ===
            # Check for dangerous API combinations
            import_cats = self.results.get("import_categories", {}).get("categories", {})
            ...
            # Crypto + File ops = ransomware-like
            if crypto_count >= 2:
    ...
    def detect_contradictions(self):
        """
        Detect contradictions that justify suspicion despite file role.
        This is the core of justificatory scoring - malicious verdict requires
        at least one contradiction between claimed role and actual behavior.
        """
        try:
            contradictions = []
            role = self.results.get("file_role", {}).get("role", "unknown")
            
            # Get relevant analysis results
            auth = self.results.get("authenticode", {})
            version = self.results.get("version_info", {})
            ...
            # === CONTRADICTION 1: Installer + No Version Info + Unsigned ===
            if role == "installer":
            ...
            # === CONTRADICTION 2: Application + Process Injection ===
            # Fix 5: Exempt signed apps from trusted vendors
            company = str(version.get("company_name", "")).lower()
            ...
            # === CONTRADICTION 3: Signed File + Malicious API Clusters ===
            if auth.get("is_signed"):
            ...
            # === CONTRADICTION 4: Tiny File + Network + Encryption ===
            ...
            # === CONTRADICTION 5: Tool Claims + No Real Functionality ===
            if role == "application":
            ...
            # === CONTRADICTION 6: Enterprise Claim + No Signature ===
            # Fix 2: System tools get reduced weight for fake_vendor
            company = str(version.get("company_name", "")).lower()
            ...
            if any(tc in company for tc in trusted_companies):
                if not auth.get("is_signed"):
                    # Reduce weight for system tools (they may be unsigned copies from Windows)
            ...
            # === CONTRADICTION 7: High Entropy Everywhere (unusual even for installers) ===
    ...
    def calculate_risk_score(self):
        """
        JUSTIFICATORY RISK SCORING - Hybrid model with baseline + contradictions
        
        Philosophy:
        - Phase 0: Baseline structural anomalies (always apply)
        - Phase 1+: Contradiction-based scoring for malicious verdict
        - Trust tiers ONLY apply if no contradictions
        
        Updated: 2026-01-21 (v3.0 - baseline scoring + FN/FP fixes)
        """
        try:
            role = self.results.get("file_role", {}).get("role", "unknown")
            ...
            # === PHASE 0: BASELINE STRUCTURAL SCORING (Always apply) ===
            # These indicators suggest suspicious files INDEPENDENT of contradictions
            baseline_score = 0
            ...
            # Unsigned + no version info = suspicious identity
            if not is_signed and not has_version:
                if role == "unknown":
                    baseline_score += 25  # Unknown file with no identity
                    risk_score["baseline_indicators"].append("No signature + no version info (unknown role): +25")
                elif not is_installer:
                    baseline_score += 15  # Non-installer with no identity
            ...
            # Small PE file with minimal imports = potential dropper/loader
            ...
            # Crypto APIs (encryption capability)
            ...
            # Screen locker APIs (window manipulation/lockout)
            ...
            # === LOCKER/RANSOMWARE PATTERN ===
            # Crypto + File enum + File mod = ransomware behavioral pattern
            # Fix 1: Exempt crypto system tools (cipher.exe, certutil.exe)
            ...
            if has_crypto and has_file_enum and current_filename not in CRYPTO_SYSTEM_TOOLS:
                locker_score = 30
                if has_file_mod:
                    locker_score += 15  # Complete ransomware capability
                if not is_signed and role == "unknown":
                    locker_score += 10  # Unknown + unsigned escalation
                total_score += locker_score
                risk_score["indicators"].append(f"LOCKER PATTERN (crypto+file_enum): +{locker_score}")
            
            # === SCREEN LOCKER PATTERN ===
            # Window manipulation + persistence OR screen APIs + no identity
            if has_screen_lock and role == "unknown" and not is_signed:
                screen_score = 25
                if has_persist:
                    screen_score += 15  # Persistent screen locker
                total_score += screen_score
                risk_score["indicators"].append(f"SCREEN LOCKER PATTERN (window hooks/lockout): +{screen_score}")
            
            # === LOADER/DROPPER PATTERN ===
            # Small file + creates processes/downloads = likely loader
            if file_size < 100000 and role == "unknown":  # < 100KB
                if has_loader and has_download:
                    ...
                    # Partial injection - just VirtualAllocEx on unknown unsigned file 
                    total_score += 20
                    risk_score["indicators"].append("SUSPICIOUS ALLOC (VirtualAllocEx, small unsigned): +20")
            
            # === MINIMAL MALWARE ESCALATION ===
            # Very small PE with suspicious API combo or persistence but no identity = escalate
            if is_pe and file_size < 50000 and not is_signed and role == "unknown":
                ...
                if suspicious_api_count >= 2:
                    ...
                    # Single persistence API on tiny unknown file is suspicious
                    total_score += 15
                    risk_score["indicators"].append("MINIMAL MALWARE (small+unsigned+persistence): +15")
            
            # === PHASE 1: GET CONTRADICTIONS (Primary driver of suspicion) ===
            ...
            # === PHASE 2: API CLUSTER ANALYSIS (Intent-based) ===
            api_clusters = self.results.get("api_clusters", {})
            ...
            # === PHASE 3: YARA MATCHES (Strong signal) ===
            yara = self.results.get("yara_matches", {})
            ...
            if yara_count > 0:
                ...
                # No YARA hits on unknown file LOWERS malware confidence
                risk_score["trust_adjustments"].append("No YARA matches: reduced malware confidence")
            
            # === PHASE 4: LIMITED ACCUMULATIVE SCORING (only if contradictions exist) ===
            # This prevents benign files from accumulating FP points
            if has_contradictions:
                # TLS callbacks (evasion indicator)
                tls = self.results.get("tls_callbacks", {})
                ...
                # Anti-analysis techniques (only significant if contradictions exist)
            ...
            if vt.get("detected") and vt_positives > 10:
                # Fix 4: Increase VT weight cap from 30 to 50 for high detection
                if vt_positives > 30:
                ...
                # Fix 3: Revoke trust for signed files with high VT detection
            ...
            # === PHASE 5: TRUST TIER ADJUSTMENTS (GUARDRAIL: only if NO contradictions AND VT not flagged) ===
            auth = self.results.get("authenticode", {})
            is_signed = auth.get("is_signed", False)
            signer = str(auth.get("signer", "")).lower()
            
            if not has_contradictions and not vt_trust_revoked:
                # Trust tiers - major vendors get significant reduction
                TRUST_TIERS = {
                ...
                for vendor, reduction in TRUST_TIERS.items():
                    if vendor in signer:
                        total_score = max(0, total_score + reduction)  # reduction is negative
                ...
                # Installer framework bonus (only if no contradictions)
                if is_installer:
            ...
            # Contradictions STRENGTHEN verdicts but are not required for high scores
            if total_score >= 70:
                ...
                if is_installer or is_admin_tool or is_system_tool:
                    ...
                    risk_score["verdict"] = "Unrated"  # Changed from "Likely Clean"
    ...
    def generate_suspicion_report(self):
        """Generate 'Why it is suspicious' explanations for detected indicators"""
        try:
            report = {
                "suspicious_dlls": [],
                "suspicious_apis": [],
            ...
            dll_explanations = {
                ...
                # shell32.dll REMOVED - it's standard Windows GUI/shell API, all installers use it
            ...
            # Suspicious API explanations (detailed)
            api_explanations = {
                # Process Injection
            ...
            # Check all imports against API explanations
            ...
            # Behavior explanations from capability mapping
            ...
            for behavior in cap.get("detected_behaviors", []):
                if behavior in behavior_explanations:
                    ...
                    report["suspicious_behaviors"].append({
                        ...
                        "evidence_apis": api_list[:5]  # Limit to 5
                    })
...
class RemoteExecutor:
    """Execute commands on remote EC2 instance via SSH"""
    ...
    try:
        ...
        if args.local:
            ...
            if args.output_file:
                ...
                # Use SHA256 hash as filename for better identification
            ...
            # Automatically parse to TXT report
            try:
            ...
            # AWS-based analysis is handled by orchestrator, skip it here for the local script
